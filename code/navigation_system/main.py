...
from .visualization import plot_trajectory, plot_pos_errors, plot_bias, plot_yaw_error, plot_altitude_profile, plot_error_norm, plot_attitude_errors
...
def run(config_path: str):
    ...
    avg_step_ms = (t1_perf - t0_perf) * 1000.0 / N
    ...
    out_dir = Path(__file__).resolve().parent/"data"/"output"/scenario
    ...
    # Save RMSE table
    save_csv(str(out_dir/"timing.csv"),
             header=["scenario", "N_steps", "avg_step_ms"],
             data=np.array([[scenario, str(N), f"{avg_step_ms:.4f}"]], dtype=object))
    ...
    plot_altitude_profile(t, truth.p_ned, p_ins, p_ekf, str(out_dir/"fig_altitude.png"))
    plot_error_norm(t, e_p_ins, e_p_ekf, str(out_dir/"fig_pos_error_norm_last60.png"), last_seconds=60.0)
    # В этой демонстрации используется упрощённая модель ориентации (yaw).
    e_roll = np.zeros_like(e_yaw_ins)
    e_pitch = np.zeros_like(e_yaw_ins)
    plot_attitude_errors(t, e_yaw_ekf, e_roll, e_pitch, str(out_dir/"fig_attitude_errors.png"))
    ...
    print(f"Среднее время шага EKF (predict+update): {avg_step_ms:.3f} мс")
    print(f"Outputs saved to: {out_dir}")

if __name__ == "__main__":
    # Можно передать путь к YAML-конфигу аргументом командной строки.
    # Пример: python -m navigation_system.main navigation_system/config_uav_orbit.yaml
    cfg = sys.argv[1] if len(sys.argv) > 1 else str(Path(__file__).resolve().parent/"config.yaml")
    run(cfg)
